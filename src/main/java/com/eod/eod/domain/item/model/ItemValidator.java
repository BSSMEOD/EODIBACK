...
public final class ItemValidator {
    ...
    public static void validateFoundAt(LocalDateTime foundAt) {
        if (foundAt == null) {
            throw new ItemBadRequestException("습득 일자는 필수입니다.");
        }
        if (foundAt.isAfter(LocalDateTime.now())) {
            throw new ItemBadRequestException("과거 날짜만 등록할 수 있습니다.");
        }
    }
    ...
    public static void validateCategory(Item.ItemCategory category) {
        if (category == null) {
            throw new ItemBadRequestException("카테고리는 필수입니다.");
        }
    }
    ...
    public static Long requirePlaceId(Long placeId) {
        if (placeId == null) {
            throw new ItemBadRequestException("장소 ID는 필수입니다.");
        }
        return placeId;
    }
    ...
    public static String requireText(String value, String message) {
        if (value == null) {
            throw new ItemBadRequestException(message);
        }
    ...
    public static void validateImage(long size, String contentType) {
        if (size > MAX_IMAGE_SIZE_BYTES) {
            throw new ItemBadRequestException("이미지는 최대 5MB까지 업로드할 수 있습니다.");
        }
        if (contentType == null || !ALLOWED_IMAGE_CONTENT_TYPES.contains(contentType)) {
            throw new ItemBadRequestException("이미지는 JPEG 또는 PNG 형식만 허용됩니다.");
        }
    }
}
